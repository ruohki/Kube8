...
Webserver::Webserver(uint16_t port) {
  ...
  _websocket->onEvent([this](AsyncWebSocket * server, AsyncWebSocketClient * client, AwsEventType type, void * arg, uint8_t *data, size_t len) { this->onWsEvent(server, client,type, arg, data, len ); });

  _webserver->addHandler(_websocket);

  _webserver->on("/heap", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/plain", String(ESP.getFreeHeap()));
  });

  _webserver->onNotFound([this](AsyncWebServerRequest *request) { this->onNotFound(request); });
  _webserver->serveStatic("/", SPIFFS, "/").setDefaultFile("index.html");

  _webserver->onRequestBody([](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total){
    if(!index)
